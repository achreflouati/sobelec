...
class SobelecItemManager {
                ...
                <div class="page-header">
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <p class="text-muted">Interface Aperion de gestion des articles</p>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="stat-card bg-warning">
                                <div class="stat-number" id="total-value-${this.page_id}">0 TND</div>
    ...
    async load_items() {
        ...
        try {
            ...
            if (response.success) {
                ...
                await this.fetch_item_prices();
                await this.fetch_additional_item_data();
                // await this.fetch_items_with_taxes();
                
                await this.render_items();
    ...
    async render_items() {
        ...
        if (this.current_items.length === 0) {
            tbody.innerHTML = this.get_no_items_html();
            return;
        }

        // Fetch stock data for all items first
        await this.fetch_stock_data_for_items();
        
        // Then render all rows
        tbody.innerHTML = this.current_items.map(item => this.get_item_row_html(item)).join('');
        
        this.setup_row_event_handlers();
    }
    
async fetch_item_prices() {
    try {
        const itemCodes = this.current_items.map(item => item.item_code);
        
        // Récupération des prix depuis la table Item Price
        ...
        const pricesByItem = prices.reduce((acc, price) => {
            if (!acc[price.item_code]) {
                acc[price.item_code] = [];
            }
            acc[price.item_code].push({
        ...
        this.current_items.forEach(item => {
            item.selling_prices = pricesByItem[item.item_code] || [];
            item.standard_selling_rate = item.selling_prices[0]?.price_list_rate || 0;
        });
        ...
        this.current_items.forEach(item => {
            item.selling_prices = [];
            item.standard_selling_rate = 0;
        });
    }
}
async fetch_items_with_taxes() {
    try {
        const itemCodes = this.current_items.map(item => item.item_code);
        const items = [];

        for (const item_code of itemCodes) {
            let doc = await frappe.db.get_doc('Item', item_code);
            items.push(doc);
        }

        return items;
    } catch (error) {
        console.error("Erreur récupération items avec taxes :", error);
        return [];
    }
}


async fetch_additional_item_data() {
    try {
        const itemCodes = this.current_items.map(item => item.item_code);
        ...
        const additionalDataMap = items.reduce((map, item) => {
            map[item.name] = {
                image: item.image,
                emplacement: item.custom_emplacement,
                ref_fournisseur: item.custom_référence_fournisseur,
        ...
        // Fusion des données
        this.current_items.forEach(item => {
            const additionalData = additionalDataMap[item.item_code] || {};
            Object.assign(item, additionalData);
        });

    } catch (error) {
        console.error("Erreur récupération données supplémentaires:", error);
        // Valeurs par défaut
        this.current_items.forEach(item => {
            item.image = null;
            item.emplacement = null;
            item.ref_fournisseur = null;
    ...
    get_item_row_html(item) {
        ...
        const reservedStock = parseFloat(item.total_reserved || 0).toFixed(2);
        const availableStock = parseFloat(item.available_stock || 0).toFixed(2);
        const sellingPrice = parseFloat(item.standard_selling_rate || 0).toFixed(2);
        const buyingPrice = parseFloat(item.standard_buying_rate || 0).toFixed(2);

        return `
            <tr data-item-code="${item.item_code}">
            <td>
                <img src="${item.image || '/assets/sobelec_extension/images/default_item.png'}" 
                     class="item-image" 
                ...
                <td class="${stockClass}"><strong>${parseFloat(item.total_stock || 0).toFixed(2)}</strong></td>
                <td>${reservedStock}</td>
                <td>${availableStock}</td>
                <td>${item.emplacement}</td>
                <td>${item.ref_fournisseur} TND</td>
                <td>${sellingPrice} TND</td>
                <td>${stockValue} TND</td>
    ...
    async fetch_stock_data_for_items() {
        const itemCodes = this.current_items.map(item => item.item_code);
        const stockData = await this.get_bin_data_for_items(itemCodes);
        
        this.current_items.forEach(item => {
            const itemStock = stockData[item.item_code] || {
                total_stock: 0,
                total_reserved: 0,
                available_stock: 0
            };
            Object.assign(item, itemStock);
        });
    }

    async get_bin_data_for_items(itemCodes) {
    try {
        // Découpage en lots de 500 articles pour éviter les requêtes trop longues
        const chunkSize = 500;
        const result = {};
        ...
        for (let i = 0; i < itemCodes.length; i += chunkSize) {
            ...
            bins.forEach(bin => {
                if (!result[bin.item_code]) {
                    result[bin.item_code] = {
                        total_stock: 0,
                        total_reserved: 0,
                        available_stock: 0
                    };
                ...
                result[bin.item_code].total_stock += (bin.actual_qty || 0);
                result[bin.item_code].total_reserved += (bin.reserved_qty || 0);
                result[bin.item_code].available_stock += (bin.projected_qty || 0);
            });
        }
        
        return result;
    } catch (error) {
        this.handle_error("Erreur chargement données stock", error);
        return {};
    }
}
    ...
    setup_row_event_handlers() {
        ...
        document.querySelectorAll('.item-info-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (!itemCode) {
                    console.error("No item code found for info button");
                    return;
                }
    ...
    render_item_details_modal(data) {
        console.log("Rendering item details modal", data);
        const image = data.image || '/assets/sobelec_extension/images/default_item.png';
        const emplacement = data.custom_emplacement || '-';
        const refFournisseur = data.custom_référence_fournisseur || '-';
        const item = data.basic_info;
        const stock = data.stock_info;
        const pricing = data.pricing_info;
        const sellingPrice = pricing.selling_prices && pricing.selling_prices.length > 0 
        ? pricing.selling_prices[0].price_list_rate.toFixed(2) + ' TND'
        : 'Non défini';
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="text-center mb-3">
                <a href="${item.image}" target="_blank" data-toggle="modal" data-target="#imageZoomModal">
        ...
        <div class="col-md-6">
            ...
            <table class="table table-sm">
                ...
                <tr><td><strong>Prix Vente:</strong></td><td>${sellingPrice} TND</td></tr>
                <tr><td><strong>Prix Achat:</strong></td><td>${pricing.standard_buying_rate} TND</td></tr>
...
<div class="modal fade" id="imageZoomModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-transparent border-0">
            <div class="modal-body p-0">
                <img src="${item.image}" class="img-fluid w-100" style="object-fit:contain;">
            </div>
        </div>
    </div>
</div>
    ...
    ${item.description ? `<div class="mt-3"><h6>Description</h6><p></p></div>` : ''}
</div>
